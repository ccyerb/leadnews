...
public class ApArticleServiceImpl  extends ServiceImpl<ApArticleMapper, ApArticle> implements ApArticleService {
    ...
    public void updateScore(ArticleVisitStreamMess mess) {
        //1.更新文章的阅读、点赞、收藏、评论的数量到數據庫
        //這一步很重要，把每個操作更新到數據庫而不是redis，以確保為文章分值乘以3后同步到redis裏的是每次正確的數據
    ...
    private void replaceDataToRedis(ApArticle apArticle, Integer score, String s) {
        //重緩存中獲取熱門文章
        String articleListStr = cacheService.get(s);//s=redis key
        if (StringUtils.isNotBlank(articleListStr)) {
            List<HotArticleVo> hotArticleVoList = JSON.parseArray(articleListStr, HotArticleVo.class);
            //如果已經是熱門文章，flag=false，默認true
            boolean flag = true;
            ...
            //只有缓存中不存在會觸發，首先查询缓存中分值最小的一条数据，进行分值的比较，如果当前文章的分值大于缓存中的数据，就替换，否則插入
            if (flag) {
                if (hotArticleVoList.size() >= 30) {
                    //排序並倒序，要排序嗎？redis 是有序的，好像
            ...
            //再次排序， (lastHot.getScore() < score)沒有排序,最後缓存到redis，
            hotArticleVoList = hotArticleVoList.stream().sorted(Comparator.comparing(HotArticleVo::getScore).reversed()).collect(Collectors.toList());
    ...
    private ApArticle updateArticle(ArticleVisitStreamMess mess) {
        //get&update related value from db
